...
export function updateGame(s: GameState, callbacks: GameCallbacks) {
    ...
    for (const o of s.orbitals) { o.angle += 0.05; const ox = p.x + Math.cos(o.angle) * o.dist, oy = p.y + Math.sin(o.angle) * o.dist; const nearby = s.spatialGrid.queryRadius(ox, oy, 100); for (const e of nearby) { if (e.type === 'projectile' || e.dead || !e.active) continue; const r = e.size + 10; if (Utils.distSq(ox, oy, e.x, e.y) < r * r) { e.hp -= 2; e.hitFlash = 2; createExplosion(s, e.x, e.y, '#00ffff', 1, 0.5); } } }
